# HYDROID — /Lotus/Powersuits/Pirate/Pirate

##P Enemies damaged by Hydroid are more vulnerable to <DT_CORROSIVE> Corrosive Status, with initial Status reducing Armor by 50%.

## /Lotus/Powersuits/PowersuitAbilities/CannonBarrageAbility
// 1 - TEMPEST BARRAGE
Drain: <ENERGY> 25 $EFFICIENCY
Radius: 10m
Duration: 10s $DURATION
Damage: <DT_CORROSIVE> 300 $STRENGTH
Explosion Radius: 5m $RANGE
#### VIRAL TEMPEST
Status Chance: <DT_VIRAL> 100% $STRENGTH


## /Lotus/Powersuits/PowersuitAbilities/TidalWaveAbility
// 2 - TIDAL SURGE
Drain: <ENERGY> 50 $EFFICIENCY
Range: 6m $RANGE
Damage: <DT_CORROSIVE> 300 $STRENGTH
#### TIDAL IMPUNITY

## /Lotus/Powersuits/PowersuitAbilities/PirateArmourAbility
// 3 - PLUNDER
Drain: <ENERGY> 75 $EFFICIENCY
Range: 25m $RANGE
Duration: 25s $DURATION
Armor / Enemy: 40 $STRENGTH
Max Armor: 750 $STRENGTH
Damage / Enemy: <DT_CORROSIVE> 10% $STRENGTH
Damage / <DT_COSSORIVE> Status Effect: 10% $STRENGTH
Max <DT_CORROSIVE> Corrosive Damage: 200% $STRENGTH
#### ROUSING PLUNDER
Health: 50

## /Lotus/Powersuits/PowersuitAbilities/KrakenAbility
// 4 - TENTACLE SWARM
Drain: <ENERGY> 50 $EFFICIENCY
Radius: 15m $RANGE
Number Of Tentacles: 20
Damage: <DT_CORROSIVE> 200 $STRENGTH
Duration: 20s $DURATION
#### PILFERING SWARM
Drop Chance: 100%