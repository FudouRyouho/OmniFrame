//! UPDATE NEEDED

# TRINITY — /Lotus/Powersuits/Trinity/Trinity

##P Allies in <AFFINITY> Affinity Range gain 50% of Trinity's Max Energy as Health.

## /Lotus/Powersuits/PowersuitAbilities/WellOfLifeAbility
// 1 - WELL OF LIFE
Drain: <ENERGY> 25 $EFFICIENCY
Range: 100m $RANGE
Duration: 20s $DURATION
Radius: 20m $RANGE
Health / Second: 100 $STRENGTH
Life Steal 1% $STRENGTH
Immortality Cooldown: <TIMER>: 60
Immortality Duration: 5s
Max Targets: 3
#### POOL OF LIFE

## /Lotus/Powersuits/PowersuitAbilities/EnergyVampireAbility
// 2 - ENERGY VAMPIRE
Drain: <ENERGY> 50 $EFFICIENCY
Energy / Pulse: <ENERGY> 25 $STRENGTH
Pulse Interval: 2,25s
Range: 100m $RANGE
Radius: 25m $RANGE
Duration: 9s $DURATION
Damage: <DT_TRUE> 25% $STRENGTH
#### VAMPIRE LEECH
Energy Conversion: 225%

## /Lotus/Powersuits/PowersuitAbilities/LinkAbility
// 3 - LINK
Drain: <ENERGY> 75 $EFFICIENCY
Enemies Affected: 3
Damage Redirection: 75%
Radius: 20m $RANGE
Duration: 20s $DURATION
#### ABATING LINK
Armor Reduction: 60% $STRENGTH //! no have cap

## /Lotus/Powersuits/PowersuitAbilities/BlessingAbility
// 4 - BLESSING
Drain: <ENERGY> 100 $EFFICIENCY
Duration: 15s $DURATION
Restoration: 80% $STRENGTH
Damage Reduction: 50% $STRENGTH
#### CHAMPION'S BLESSING
