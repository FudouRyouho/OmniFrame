//! UPDATE NEEDED

# KOUMEI — /Lotus/Powersuits/Koumei/Koumei

##P

## /Lotus/Powersuits/PowersuitAbilities/KoumeiStringsAbility
// 1 - KUMIHIMO
Drain: <ENERGY> 25 $EFFICIENCY
Damage: 25x <KOUMEI_DICE_1> $STRENGTH
Range: 30m $RANGE
Duration: 12s $DURATION
Thread Length: 15m $RANGE

## /Lotus/Powersuits/PowersuitAbilities/KoumeiFortuneAbility
// 2 - 
#### OMIKUJI'S FORTUNE
Drain: <ENERGY> 50 $EFFICIENCY
Rare Decree Chance: 5%
Cooldown Cap: 150s

## /Lotus/Powersuits/PowersuitAbilities/KoumeiBlockAbility
// 3 - 
Drain: <ENERGY> 75 $EFFICIENCY
Block Chance: 50%
Heal Multiplier: 1x
Recast Count: 5
Internal Cooldown: <TIMER> 3s
Number Of Charms: 1x <KOUMEI_DICE_1>

## /Lotus/Powersuits/PowersuitAbilities/KoumeiPuppetAbility
// 4 - BUNRAKU
Drain: <ENERGY> 100 $EFFICIENCY
Cooldown: <TIMER> 3
Damage: <DT_PUNCTURE> 500 $STRENGTH
Range: 30m $RANGE
Duration: 20s $DURATION
Status / Enemy: 1x <KOUMEI_DICE_1>
