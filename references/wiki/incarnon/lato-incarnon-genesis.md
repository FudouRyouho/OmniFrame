# Lato — Incarnon Genesis

> **Variantes:** Lato · Lato Vandal · Lato Prime
> Fuente: https://wiki.warframe.com/w/Lato_Incarnon_Genesis
> Fuente actualizada: 2026-06-23
> Raw: lato-incarnon-genesis.wikitext

---

## EVO 1 — Incarnon Form

![incarnon_form](../../../Project/public/assets/incarnon-evolutions/IncarnonFormRicochetOnHittingEnemies(xWhite).png)

**Descripción:**
- Weakpoint hits charge Incarnon Transmutation; Alt Fire transmutes. Switching back will expend any remaining charge.
- Gain Punch Through and Ricochet.

**Notas:**
- Incarnon Form gains increased base damage, Critical Chance, Critical Multiplier, and overall Status Chance. It also gains an innate Multishot of 2 (similar to shotguns) and 0.8 meter Punch Through. Shots also ricochet to **1** nearby enemy within **10** meters (**12** meters for the prime) of the main target, plus any subsequent targets struck via punch through. However, it has slower Fire Rate and significantly higher Recoil.
- Modding for Punch Through and hitting multiple targets will create successive ricochets.
- <DT_VOID> Status Effect prevents ricochets from occurring.

---

## EVO 2

**Challenge:** Complete a solo mission with this weapon equipped

### Haven Foray

![haven_foray](../../../Project/public/assets/incarnon-evolutions/DamagePlusXDamageWithOvershields(xWhite).png)

**Efectos:**
- Increase Base Damage by **+X**.
- With Overshields: Increase Base Damage by **+Y**.

| Variante | Valor |
|----------|-------|
| Lato | X = 40 / Y = 40 |
| Lato Vandal | X = 22 / Y = 40 |
| Lato Prime | X = 2 / Y = 34 |

### Reified Bane

![reified_bane](../../../Project/public/assets/incarnon-evolutions/ReloadFromEmpty(xWhite).png)

**Efectos:**
- Increase Base Damage by **+X**.
- On Reload from Empty Magazine: Increase Base Damage by **+Y**.

| Variante | Valor |
|----------|-------|
| Lato | X = 30 / Y = 30 |
| Lato Vandal | X = 12 / Y = 30 |
| Lato Prime | X = 2 / Y = 24 |

**Notas:**
- Once the damage bonus has been earned, the ONLY way to lose it is to manually reload with a non-empty magazine.
- The bonus will be retained through Incarnon transformations, regardless of magazine status when transformed.

---

## EVO 3

**Challenge:** Kill **100** enemies with this weapon's Incarnon Form

### Void's Guidance

![voids_guidance](../../../Project/public/assets/incarnon-evolutions/LessRecoilMoreAccuracy(xWhite).png)

**Efectos:**
- **+50%** Accuracy
- **-50%** Recoil

| Variante | Valor |
|----------|-------|
| Lato | - |
| Lato Vandal | - |
| Lato Prime | - |

### Marksman's Gain

![marksmans_gain](../../../Project/public/assets/incarnon-evolutions/OnHeadshotKillAmmoEff(xWhite).png)

**Efectos:**
- On Headshot: **20%** chance for **+50%** Ammo Efficiency for 6s.

| Variante | Valor |
|----------|-------|
| Lato | - |
| Lato Vandal | - |
| Lato Prime | - |

**Notas:**
- The bonus can be refreshed on subsequent headshots.
- The bonus does not affect the Incarnon form.

### Exact Penance

![exact_penance](../../../Project/public/assets/incarnon-evolutions/ChanceToReplenishMagOnHeadshot(xWhite).png)

**Efectos:**
- On Kill: **50%** chance for Instant Reload.

| Variante | Valor |
|----------|-------|
| Lato | - |
| Lato Vandal | - |
| Lato Prime | - |

**Notas:**
- Kills from status effects can also trigger the effect.
- The bonus does not affect the Incarnon form.

---

## EVO 4

**Challenge:** Get **40** headshots in a single mission

### Survivor's Edge

![survivors_edge](../../../Project/public/assets/incarnon-evolutions/XCritChanceYStatChance(xWhite).png)

**Efectos:**
- Increase Base Critical Chance by **+X%**.
- Increase Base Status Chance by **+Y%**.

| Variante | Valor |
|----------|-------|
| Lato | X = 20% / Y = 12% |
| Lato Vandal | X = 6% / Y = 12% |
| Lato Prime | X = 4% / Y = 10% |

### Deathtrap Trigger

![deathtrap_trigger](../../../Project/public/assets/incarnon-evolutions/OnEquipCritChanceDamageOnFirstAttack(xWhite).png)

**Efectos:**
- On Equip from Primary:
- Increase Base Critical Chance by **+X%** for **4s**.
- Increase Base Critical Damage Multiplier by **+Y** for **4s**.

| Variante | Valor |
|----------|-------|
| Lato | X = 50% / Y = 3.2x |
| Lato Vandal | X = 38% / Y = 2.6x |
| Lato Prime | X = 30% / Y = 2.3x |

### Carnage Reign

![carnage_reign](../../../Project/public/assets/incarnon-evolutions/DamagePerActiveStatus(xWhite).png)

**Efectos:**
- **+40%** Direct Damage per Status Type affecting the target.

| Variante | Valor |
|----------|-------|
| Lato | - |
| Lato Vandal | - |
| Lato Prime | - |
