# Vasto — Incarnon Genesis

> **Variantes:** Vasto · Prime
> Fuente: https://wiki.warframe.com/w/Vasto_Incarnon_Genesis
> Fuente actualizada: 2026-06-01
> Raw: vasto-incarnon-genesis.wikitext

---

## EVO 1 — Incarnon Form

![incarnon_form](../../../../../Project/public/assets/incarnon-evolutions/HammerFanBurstShotgun(xWhite).png)

**Descripción:**
- Weakpoint hits charge Incarnon Transmutation; Alt Fire transmutes. Switching back will expend any remaining charge.
- Rapidly empty a full magazine.

**Notas:**
- Incarnon Form transforms into a 6-round burst with **6** base multishot along with increased Critical Chance and Critical Multiplier. However, it has reduced base damage and Status Chance per bullet, has significantly higher Recoil, and cannot Zoom.

---

## EVO 2

**Challenge:** Complete a solo mission with this weapon equipped

### Lone Gun

![lone_gun](../../../../../Project/public/assets/incarnon-evolutions/DamageWithNoPrimaryOrMeleeEquipped(xWhite).png)

**Efectos:**
- Increase Base Damage by **+X**.
- With No Primary Equipped:
- Increase Base Damage by **+40**
- Increase Base Magazine Capacity by **+14**.

| Variante | Valor |
|----------|-------|
| Vasto | X = 66 |
| Prime | X = 24 |

**Notas:**
- Increased Base Magazine Capacity does not affect Incarnon Form.

### Deathtrap Trigger

![deathtrap_trigger](../../../../../Project/public/assets/incarnon-evolutions/CritChanceAndDmgOnFirstShot(xWhite).png)

**Efectos:**
- Increase Base Damage by **+X**.
- On Equip From Primary:
- Increase Base Critical Chance by **+30%** for **3s**.
- Increase Base Critical Damage Multiplier by **+Y** for **3s**.

| Variante | Valor |
|----------|-------|
| Vasto | X = 66 / Y = 2.2x |
| Prime | X = 24 / Y = 0.8x |

---

## EVO 3

**Challenge:** Kill **100** enemies with this weapon's Incarnon Form

### Extended Volley

![extended_volley](../../../../../Project/public/assets/incarnon-evolutions/IncarnonClipSize(xWhite).png)

**Efectos:**
- Increase Base Magazine Capacity by **+6**.

| Variante | Valor |
|----------|-------|
| Vasto | - |
| Prime | - |

**Notas:**
- Increased Base Magazine Capacity does not affect Incarnon Form.

### Marksman's Hand

![marksmans_hand](../../../../../Project/public/assets/incarnon-evolutions/LessRecoil(xWhite).png)

**Efectos:**
- **-50%** Weapon Recoil.

| Variante | Valor |
|----------|-------|
| Vasto | - |
| Prime | - |

### Awakened Readiness

![awakened_readiness](../../../../../Project/public/assets/incarnon-evolutions/IncarnonReloadSpeed(xWhite).png)

**Efectos:**
- **+20%** Magazine Reloaded per second when Holstered.

| Variante | Valor |
|----------|-------|
| Vasto | - |
| Prime | - |

**Notas:**
- Does not apply to Incarnon Form's Magazine.

---

## EVO 4

**Challenge:** Kill **30** enemies while airborne

### Commodore's Fortune

![commodores_fortune](../../../../../Project/public/assets/incarnon-evolutions/HighCrit(xWhite).png)

**Efectos:**
- Increase Base Critical Chance by **+X%**.

| Variante | Valor |
|----------|-------|
| Vasto | X = 16% |
| Prime | X = 18% |

### Survivor's Edge

![survivors_edge](../../../../../Project/public/assets/incarnon-evolutions/XCritChanceYStatChance(xWhite).png)

**Efectos:**
- Increase Base Critical Chance by **+10%**.
- Increase Base Status Chance by **+10%**.

| Variante | Valor |
|----------|-------|
| Vasto | - |
| Prime | - |

**Notas:**
- Status Chance increase for Incarnon Form is divided by the innate multishot.

### Reaper's Plenty

![reapers_plenty](../../../../../Project/public/assets/incarnon-evolutions/OnHeadshotAmmoEfficiency(xWhite).png)

**Efectos:**
- On Headshot: **+100%** Ammo Efficiency for **2s**.

| Variante | Valor |
|----------|-------|
| Vasto | - |
| Prime | - |

**Notas:**
- Does not apply to Incarnon Form.
