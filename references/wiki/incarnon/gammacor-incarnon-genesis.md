# Gammacor — Incarnon Genesis

> **Variantes:** Gammacor · Synoid Gammacor
> Fuente: https://wiki.warframe.com/w/Gammacor_Incarnon_Genesis
> Fuente actualizada: 2026-06-23
> Raw: gammacor-incarnon-genesis.wikitext

---

## EVO 1 — Incarnon Form

![incarnon_form](../../../../../Project/public/assets/incarnon-evolutions/ProjectileBombFreezeProc(xWhite).png)

**Descripción:**
- Weakpoint hits charge Incarnon Transmutation; Alt Fire transmutes. Switching back will expend any remaining charge.
- Fires projectiles that pull enemies into a <DT_COLD> Damage explosion.

**Notas:**
- Incarnon Form shoots semi-automatic projectiles that deal guaranteed <DT_IMPACT> proc on direct impacts, then pulls and staggers enemies within **5** meters for **1.2** seconds before exploding, inflicting pure **760** <DT_COLD> damage with a guaranteed <DT_COLD> proc on the explosion. However it has significantly slower Fire Rate and the explosion possesses Damage Falloff from 100% to 70% from central impact.

---

## EVO 2

**Challenge:** Complete a solo mission with this weapon equipped

### Sage's Resolve

![sages_resolve](../../../../../Project/public/assets/incarnon-evolutions/DamageAndMultishotWithActiveChanneledAbility(xWhite).png)

**Efectos:**
- Increase Base Damage by **+10**.
- With Channeled Ability Active: **+25%** Multishot.

| Variante | Valor |
|----------|-------|
| Gammacor | - |
| Synoid Gammacor | - |

### Infused Shots

![infused_shots](../../../../../Project/public/assets/incarnon-evolutions/DamageWithEnergySpent(xWhite).png)

**Efectos:**
- Increase Base Damage by **+6**.
- On 50 Energy Spent: Increase Base Damage by **+5** for **10s**. Stacks up to **4x**.

| Variante | Valor |
|----------|-------|
| Gammacor | - |
| Synoid Gammacor | - |

**Notas:**
- All stacks are removed after the most recent one expires. Gaining a stack will refresh the duration of all stacks.

---

## EVO 3

**Challenge:** Kill **100** enemies with this weapon's Incarnon Form

### Evolved Autoloader

![evolved_autoloader](../../../../../Project/public/assets/incarnon-evolutions/ReloadWhileHolstered(xWhite).png)

**Efectos:**
- **+50%** Magazine Reloaded/s when Holstered

| Variante | Valor |
|----------|-------|
| Gammacor | - |
| Synoid Gammacor | - |

### Moonrise Velocity

![moonrise_velocity](../../../../../Project/public/assets/incarnon-evolutions/MoreRange(xWhite).png)

**Efectos:**
- **+8** Range

| Variante | Valor |
|----------|-------|
| Gammacor | - |
| Synoid Gammacor | - |

### Extended Volley

![extended_volley](../../../../../Project/public/assets/incarnon-evolutions/IncarnonClipSize(xWhite).png)

**Efectos:**
- Increase Base Magazine Capacity by **+40**.

| Variante | Valor |
|----------|-------|
| Gammacor | - |
| Synoid Gammacor | - |

---

## EVO 4

**Challenge:** Kill **30** enemies while airborne

### Critical Parallel

![critical_parallel](../../../../../Project/public/assets/incarnon-evolutions/CritChanceCritDamage(xWhite).png)

**Efectos:**
- Increase Base Critical Chance by **+X%**.
- Increase Base Critical Damage Multiplier by **+0.2x**.

| Variante | Valor |
|----------|-------|
| Gammacor | X = 20% |
| Synoid Gammacor | X = 10% |

### Survivor's Edge

![survivors_edge](../../../../../Project/public/assets/incarnon-evolutions/XCritChanceYStatChance(xWhite).png)

**Efectos:**
- Increase Base Critical Chance by **+X%**.
- Increase Base Status Chance by **+Y%**.

| Variante | Valor |
|----------|-------|
| Gammacor | X = 12% / Y = 10% |
| Synoid Gammacor | X = 6% / Y = 6% |

### Elemental Balance

![elemental_balance](../../../../../Project/public/assets/incarnon-evolutions/HighStatus(xWhite).png)

**Efectos:**
- Increase Base Status Chance by **+X%**.

| Variante | Valor |
|----------|-------|
| Gammacor | X = 20% |
| Synoid Gammacor | X = 10% |
