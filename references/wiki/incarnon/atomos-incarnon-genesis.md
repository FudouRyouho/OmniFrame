# Atomos — Incarnon Genesis

> **Variantes:** Atomos
> Fuente: https://wiki.warframe.com/w/Atomos_Incarnon_Genesis
> Fuente actualizada: 2026-06-19
> Raw: atomos-incarnon-genesis.wikitext

---

## EVO 1 — Incarnon Form

![incarnon_form](../../../Project/public/assets/incarnon-evolutions/IncarnonFormNapalmGrenadeLauncher(xWhite).png)

**Descripción:**
- Weakpoint hits charge Incarnon Transmutation; Alt Fire transmutes. Switching back will expend any remaining charge.
- Fire high-powered grenades.

**Notas:**
- Incarnon Form becomes a semi-automatic grenade launcher whose projectiles explode in a **7.9** meter radius dealing <DT_IMPACT> damage on direct hits and <DT_BLAST> damage on the explosion, with significantly higher base damage, Critical Chance, Critical Multiplier, and Status Chance. However it has significantly slower Fire Rate, the explosion possesses linear Damage Falloff from 100% to 20% from central impact, and the projectile has travel time with arcing affected by Projectile Speed.

---

## EVO 2

**Challenge:** Complete a solo mission with this weapon equipped

### Paladin Virtue

![paladin_virtue](../../../Project/public/assets/incarnon-evolutions/IncreasedCritWithEnergy(xWhite).png)

**Efectos:**
- Increase Damage by **+24**.
- With Energy Max over 700: Increase Critical Damage Multiplier by **+1x**.

| Variante | Valor |
|----------|-------|
| Atomos | - |

### Hoplite Virtue

![hoplite_virtue](../../../Project/public/assets/incarnon-evolutions/DamageOnShieldLoss(xWhite).png)

**Efectos:**
- Increase Damage by **+24**.
- On Shield Break: Increase Damage by **+30** for **8s**.

| Variante | Valor |
|----------|-------|
| Atomos | - |

**Notas:**
- This is on **personal** shield break, not breaking enemy shields.

---

## EVO 3

**Challenge:** Kill **100** enemies with this weapon's Incarnon Form

### Moonrise Velocity

![moonrise_velocity](../../../Project/public/assets/incarnon-evolutions/MoreRange(xWhite).png)

**Efectos:**
- **+7** Range.

| Variante | Valor |
|----------|-------|
| Atomos | - |

**Notas:**
- Bonus stacks additively to Ruinous Extension
- This bonus has no effect on Incarnon Form

### Mercenary Chamber

![mercenary_chamber](../../../Project/public/assets/incarnon-evolutions/IncarnonAmmoPool(xWhite).png)

**Efectos:**
- Increase ammo capacity to **560**.

| Variante | Valor |
|----------|-------|
| Atomos | - |

**Notas:**
- This bonus not affect Incarnon form

### Resonant Restore

![resonant_restore](../../../Project/public/assets/incarnon-evolutions/IncarnonReloadSpeed(xWhite).png)

**Efectos:**
- On Reload From Empty: Increase magazine capacity by **+5**. Stacks up to **7x**.

| Variante | Valor |
|----------|-------|
| Atomos | - |

---

## EVO 4

**Challenge:** Kill **30** enemies afflicted with **4** or more status effects

### Elemental Balance

![elemental_balance](../../../Project/public/assets/incarnon-evolutions/HighStatus(xWhite).png)

**Efectos:**
- Increase Status Chance by **+20%**.

| Variante | Valor |
|----------|-------|
| Atomos | - |

### Commodore's Fortune

![commodores_fortune](../../../Project/public/assets/incarnon-evolutions/HighCrit(xWhite).png)

**Efectos:**
- Increase Critical Chance by **+19%**.

| Variante | Valor |
|----------|-------|
| Atomos | - |

### Survivor's Edge

![survivors_edge](../../../Project/public/assets/incarnon-evolutions/XCritChanceYStatChance(xWhite).png)

**Efectos:**
- Increase Critical Chance by **+10%**.
- Increase Status Chance by **+10%**.

| Variante | Valor |
|----------|-------|
| Atomos | - |
