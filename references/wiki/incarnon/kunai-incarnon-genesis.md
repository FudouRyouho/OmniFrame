# Kunai — Incarnon Genesis

> **Variantes:** Kunai · Mk1-Kunai
> Fuente: https://wiki.warframe.com/w/Kunai_Incarnon_Genesis
> Fuente actualizada: 2026-07-23
> Raw: kunai-incarnon-genesis.wikitext

---

## EVO 1 — Incarnon Form

![incarnon_form](../../../../../Project/public/assets/incarnon-evolutions/AddMultishotAndKunaiSeekHeads(xWhite).png)

**Descripción:**
- Weakpoint hits charge Incarnon Transmutation; Alt Fire transmutes. Switching back will expend any remaining charge.
- Projectiles seek Headshots and have increased Multishot.

**Notas:**
- Incarnon Form grants its projectiles head-homing properties, with increased base damage weighted towards <DT_SLASH>, Critical Chance, and Critical Multiplier, and 2 base Multishot.
- Punch Through is ignored in Incarnon Form and will not cause projectiles to home in on additional targets.
- Concealed Explosives is disabled in Incarnon Form.

---

## EVO 2

**Challenge:** Complete a solo mission with this weapon equipped

### Swift Conclusion

![swift_conclusion](../../../../../Project/public/assets/incarnon-evolutions/DamageAndAdditionalDamageToEnemiesAtHalfHealth(xWhite).png)

**Efectos:**
- Increase Damage by **+X**.
- Increase Damage by **+200%** if enemy has less than half health.

| Variante | Valor |
|----------|-------|
| Kunai | X = 70 |
| Mk1-Kunai | X = 80 |

**Notas:**
- Damage bonus if enemy has less than half health is additive with Hornet Strike in basic Kunai form, and multiplicative in Incarnon form. It is also additive with Galvanized Shot in .

### Sage's Resolve

![sages_resolve](../../../../../Project/public/assets/incarnon-evolutions/DamageAndMultishotWithActiveChanneledAbility(xWhite).png)

**Efectos:**
- Increase Base Damage by **+X**
- With Channeled Ability active: **+100%** Multishot.

| Variante | Valor |
|----------|-------|
| Kunai | X = 70 |
| Mk1-Kunai | X = 80 |

---

## EVO 3

**Challenge:** Kill **100** enemies with this weapon's Incarnon Form

### Swift Deliverance

![swift_deliverance](../../../../../Project/public/assets/incarnon-evolutions/IncarnonProjectileSpeed(xWhite).png)

**Efectos:**
- **+60%** Projectile Speed.

| Variante | Valor |
|----------|-------|
| Kunai | - |
| Mk1-Kunai | - |

### Extended Volley

![extended_volley](../../../../../Project/public/assets/incarnon-evolutions/IncarnonClipSize(xWhite).png)

**Efectos:**
- Increase Base Magazine Capacity by **+10**.

| Variante | Valor |
|----------|-------|
| Kunai | - |
| Mk1-Kunai | - |

### Rapid Reinforcement

![rapid_reinforcement](../../../../../Project/public/assets/incarnon-evolutions/IncarnonReloadSpeed(xWhite).png)

**Efectos:**
- **+50%**  Reload Speed.

| Variante | Valor |
|----------|-------|
| Kunai | - |
| Mk1-Kunai | - |

**Notas:**
- Bonus stacks additively with reload speed mods such as Quickdraw.

---

## EVO 4

**Challenge:** Kill **60** enemies within **10m**

### Deathtrap Trigger

![deathtrap_trigger](../../../../../Project/public/assets/incarnon-evolutions/OnEquipCritChanceDamageOnFirstAttack(xWhite).png)

**Efectos:**
- On equip from Primary:
- Increase Base Critical Chance by **+X%** for **4s**.
- Increase Base Critical Damage Multiplier by **+Y** for **4s**.

| Variante | Valor |
|----------|-------|
| Kunai | X = 40% / Y = 1.4x |
| Mk1-Kunai | X = 42% / Y = 1x |

**Notas:**
- Bug: These are displayed as permanent bonuses in the Arsenal but only actually apply while the 4 second Deathtrap Trigger buff is active.

### Accelerating Volley

![accelerating_volley](../../../../../Project/public/assets/incarnon-evolutions/IncarnonFireRate(xWhite).png)

**Efectos:**
- On Fire: **+20%** Fire Rate for **0.5s**. Stacks up to **5x.**

| Variante | Valor |
|----------|-------|
| Kunai | - |
| Mk1-Kunai | - |

**Notas:**
- Bonus stacks additively with fire rate mods such as Gunslinger.

### Survivor's Edge

![survivors_edge](../../../../../Project/public/assets/incarnon-evolutions/XCritChanceYStatChance(xWhite).png)

**Efectos:**
- Increase Base Critical Chance by **+12%**.
- Increase Base Status Chance by **+12%**.

| Variante | Valor |
|----------|-------|
| Kunai | - |
| Mk1-Kunai | - |

**Notas:**
- Base Status Chance increase is divided among the 2 projectiles in Incarnon mode, resulting in **+6%** Status Chance per projectile.
