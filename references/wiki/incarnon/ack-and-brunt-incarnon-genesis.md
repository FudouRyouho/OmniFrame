# Ack And Brunt — Incarnon Genesis

> **Variantes:** Ack & Brunt
> Fuente: https://wiki.warframe.com/w/Ack_&_Brunt_Incarnon_Genesis
> Fuente actualizada: 2026-06-20
> Raw: ack-and-brunt-incarnon-genesis.wikitext

---

## EVO 1 — Incarnon Form

![incarnon_form](../../../../../Project/public/assets/incarnon-evolutions/HeavyAttacksSendOutSpectralAxeAndShield(xWhite).png)

**Descripción:**
- Reach **6x** Combo and then Heavy Attack to activate Incarnon Form.
- Send forth a spectral axe during aerial and slide attacks.
- **+100%** Melee Damage
- **+20%** Sprint Speed
- **+20%** to Parkour Velocity

**Notas:**
- Spectral axes deal **149** damage on contact, then explode dealing **303** <DT_HEAT> damage with a forced <DT_IMPACT> status effect in a **5m** radius. Axes launched by aerial attacks have **2x** damage.
- <DT_HEAT> damage will combine with modded elements (for example, an Ack & Brunt modded for <DT_ELECTRICITY> will throw <DT_RADIATION> axes).
- Spectral axe hits do **not** add to the melee combo counter.
- Direct hits of spectral axes and explosions are affected by universal base damage sources like Vigorous Swap, Holster Amp, Arcane Arachne, Vex Armor and Amp.
- Direct hits of spectral axes are affected by Condition Overload, while the explosions are not.
- Direct hits of spectral axes and explosions are **not** affected by the damage increase on Evolution II perks or melee damage bonuses like Incarnon form's innate +100% melee damage, Pressure Point, and Arcane Fury.
- Lateral slam attack is **replaced** by the axe throw, reducing mobility. Slam attacks can only be performed directly downwards while Incarnon is active.

---

## EVO 2

**Challenge:** Complete a solo mission with this weapon equipped

### Templar's Wrath

![templars_wrath](../../../../../Project/public/assets/incarnon-evolutions/HeavyEfficiencyWithArmour(xWhite).png)

**Efectos:**
- Increase Base Damage by **+150**.
- With Armor Over **450**: **+70%** Heavy Attack Efficiency.

| Variante | Valor |
|----------|-------|
| Ack & Brunt | - |

### Shieldmaiden's Rush

![shieldmaidens_rush](../../../../../Project/public/assets/incarnon-evolutions/OnShieldBreakComboGainFromBlockedHits(xWhite).png)

**Efectos:**
- Increase Base Damage by **+150**.
- On Shield Break: **+300%** Combo Count Chance while Blocking for **11** seconds.

| Variante | Valor |
|----------|-------|
| Ack & Brunt | - |

---

## EVO 3

**Challenge:** Activate this weapon's Incarnon form **6** times in a mission

### Orokin Reach

![orokin_reach](../../../../../Project/public/assets/incarnon-evolutions/MoreRange(xWhite).png)

**Efectos:**
- **+0.7** Range

| Variante | Valor |
|----------|-------|
| Ack & Brunt | - |

### Abiding Hold

![abiding_hold](../../../../../Project/public/assets/incarnon-evolutions/IncarnonFireRate(xWhite).png)

**Efectos:**
- Combo Timer pauses when weapon is holstered.

| Variante | Valor |
|----------|-------|
| Ack & Brunt | - |

### Swift Break

![swift_break](../../../../../Project/public/assets/incarnon-evolutions/ComboCounterPausedWhenHolstered(xWhite).png)

**Efectos:**
- **+70%** Heavy Attack Windup Speed

| Variante | Valor |
|----------|-------|
| Ack & Brunt | - |

---

## EVO 4

**Challenge:** Reach **10x** combo **10** times

### Rogue Readiness

![rogue_readiness](../../../../../Project/public/assets/incarnon-evolutions/ChanceToReplenishMagOnHeadshot(xWhite).png)

**Efectos:**
- On Kill: **50%** chance for Primary and Secondary weapons instantly reload.

| Variante | Valor |
|----------|-------|
| Ack & Brunt | - |

### Absolute Valor

![absolute_valor](../../../../../Project/public/assets/incarnon-evolutions/HighCrit(xWhite).png)

**Efectos:**
- Increase Base Critical Chance by **+17%**.

| Variante | Valor |
|----------|-------|
| Ack & Brunt | - |

### Versatile Creed

![versatile_creed](../../../../../Project/public/assets/incarnon-evolutions/XCritChanceYStatChance(xWhite).png)

**Efectos:**
- Increase Base Critical Chance by **+9%**.
- Increase Base Status Chance by **+9%**.

| Variante | Valor |
|----------|-------|
| Ack & Brunt | - |
