# Dread — Incarnon Genesis

> **Variantes:** Dread
> Fuente: https://wiki.warframe.com/w/Dread_Incarnon_Genesis
> Fuente actualizada: 2026-06-23
> Raw: dread-incarnon-genesis.wikitext

---

## EVO 1 — Incarnon Form

![incarnon_form](../../../Project/public/assets/incarnon-evolutions/IncarnonFormIncreaseDamageAndSizeOfProjectile(xWhite).png)

**Descripción:**
- Weakpoint hits charge Incarnon Transmutation; Alt Fire transmutes. Switching back will expend any remaining charge.
- Sacrifice silence for increased projectile size and <DT_HEAT> Damage.

**Notas:**
- Incarnon Form fires a wide projectile that deals <DT_HEAT> damage, with increased base damage, Critical Multiplier, and Status Chance, and infinite body Punch Through. However, it no longer deals <DT_PUNCTURE> damage, has a headshot multiplier of 1x, has slightly slower charge time, can no longer be fired from a partial charge, and is no longer silent.

---

## EVO 2

**Challenge:** Complete a solo mission with this weapon equipped

### Hitman's Opportunity

![hitmans_opportunity](../../../Project/public/assets/incarnon-evolutions/DamageAndAdditionalDamageToEnemiesAtHalfHealth(xWhite).png)

**Efectos:**
- Increase Base Damage by **+70**.
- **+100%** Damage to enemies below half Health

| Variante | Valor |
|----------|-------|
| Dread | - |

**Notas:**
- The conditional damage bonus ignores the base damage increase from the same perk, the 2x damage from the charged shot (Primary Fire only) and Galvanized Aptitude's damage bonus.

### Stalker's Resentment

![stalkers_resentment](../../../Project/public/assets/incarnon-evolutions/IncreasedFireRateWithDespairAndHateEquipped(xWhite).png)

**Efectos:**
- Increase Base Damage by **+50**.
- With Hate and Despair Equipped: Hits increase Base Damage by **+10** and Fire Rate by **+10%**, stacks up to **5x**. Resets on missed shot.

| Variante | Valor |
|----------|-------|
| Dread | - |

**Notas:**
- Affects both regular and Incarnon forms.

---

## EVO 3

**Challenge:** Kill **100** enemies with this weapon's Incarnon Form

### Swift Deliverance

![swift_deliverance](../../../Project/public/assets/incarnon-evolutions/IncarnonProjectileSpeed(xWhite).png)

**Efectos:**
- **+30%** Projectile Speed

| Variante | Valor |
|----------|-------|
| Dread | - |

### Marksman's Focus

![marksmans_focus](../../../Project/public/assets/incarnon-evolutions/LowerFOV(xWhite).png)

**Efectos:**
- **-30%** Zoom

| Variante | Valor |
|----------|-------|
| Dread | - |

### Hitman's Hoard

![hitmans_hoard](../../../Project/public/assets/incarnon-evolutions/IncarnonAmmoPool(xWhite).png)

**Efectos:**
- Increase Base Ammo Capacity to **144**

| Variante | Valor |
|----------|-------|
| Dread | - |

**Notas:**
- Does not affect the Incarnon form.

---

## EVO 4

**Challenge:** Kill **40** enemies that are at least **40m** away

### Survivor's Edge

![survivors_edge](../../../Project/public/assets/incarnon-evolutions/XCritChanceYStatChance(xWhite).png)

**Efectos:**
- Increase Base Critical Chance by **+10%**
- Increase Base Status Chance by **+10%**

| Variante | Valor |
|----------|-------|
| Dread | - |

### Elemental Balance

![elemental_balance](../../../Project/public/assets/incarnon-evolutions/HighStatus(xWhite).png)

**Efectos:**
- Increase Base Status Chance by **+24%**

| Variante | Valor |
|----------|-------|
| Dread | - |

### Zeroed In

![zeroed_in](../../../Project/public/assets/incarnon-evolutions/CritDamage(xWhite).png)

**Efectos:**
- Increase Base Critical Damage Multiplier by **+1x**

| Variante | Valor |
|----------|-------|
| Dread | - |
