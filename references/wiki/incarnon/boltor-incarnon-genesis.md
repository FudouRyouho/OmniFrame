# Boltor — Incarnon Genesis

> **Variantes:** Boltor · Telos · Prime
> Fuente: https://wiki.warframe.com/w/Boltor_Incarnon_Genesis
> Fuente actualizada: 2026-04-30
> Raw: boltor-incarnon-genesis.wikitext

---

## EVO 1 — Incarnon Form

![incarnon_form](../../../../../Project/public/assets/incarnon-evolutions/IncarnonFormShrapnelMultishotIncreasedSlash(xWhite).png)

**Descripción:**
- Weakpoint hits charge Incarnon Transmutation; Alt Fire transmutes. Switching back will expend any remaining charge.
- Prioritize <DT_SLASH> Damage and gain Multishot.

**Notas:**
- Incarnon Form turns the weapon in a rapid-fire pseudo-shotgun with **3** base multishot, increased overall damage stats, added <DT_SLASH> damage, and slightly faster fire rate, but with lower accuracy resulting in a shotgun-like spread pattern.

---

## EVO 2

**Challenge:** Complete a solo mission with this weapon equipped

### Hunter's Mantra

![hunters_mantra](../../../../../Project/public/assets/incarnon-evolutions/DamageAccuracyPunchThroughActiveChanAbility(xWhite).png)

**Efectos:**
- Increase Base Damage by **+X**.
- With Channeled Ability active: Punch Through **+4** and **+40%** Accuracy.

| Variante | Valor |
|----------|-------|
| Boltor | X = 18 |
| Telos | X = 4 |
| Prime | X = 4 |

### Crimson Overture

![crimson_overture](../../../../../Project/public/assets/incarnon-evolutions/DamageAmmoEfficiencyOnKill(xWhite).png)

**Efectos:**
- Increase Base Damage by **+X**.
- On Kill: Increase Base Damage by **+2** and **+20%** Ammo Efficiency for **5** seconds. Stacks up to **Y**x

| Variante | Valor |
|----------|-------|
| Boltor | X = 12 / Y = 4 |
| Telos | X = 0 / Y = 3 |
| Prime | X = 0 / Y = 3 |

---

## EVO 3

**Challenge:** Kill **100** enemies with this weapon's Incarnon Form

### Swift Deliverance

![swift_deliverance](../../../../../Project/public/assets/incarnon-evolutions/IncarnonProjectileSpeed(xWhite).png)

**Efectos:**
- **+60%** Projectile Speed.

| Variante | Valor |
|----------|-------|
| Boltor | - |
| Telos | - |
| Prime | - |

### Extended Volley

![extended_volley](../../../../../Project/public/assets/incarnon-evolutions/IncarnonClipSize(xWhite).png)

**Efectos:**
- Increase Base Magazine Capacity by **+20**.

| Variante | Valor |
|----------|-------|
| Boltor | - |
| Telos | - |
| Prime | - |

### Rapid Reinforcement

![rapid_reinforcement](../../../../../Project/public/assets/incarnon-evolutions/IncarnonReloadSpeed(xWhite).png)

**Efectos:**
- **+60%** Reload Speed.

| Variante | Valor |
|----------|-------|
| Boltor | - |
| Telos | - |
| Prime | - |

---

## EVO 4

**Challenge:** Kill **40** enemies while sliding.

### Elemental Balance

![elemental_balance](../../../../../Project/public/assets/incarnon-evolutions/HighStatus(xWhite).png)

**Efectos:**
- Increase Base Status Chance by **+X%**

| Variante | Valor |
|----------|-------|
| Boltor | X = 20% |
| Telos | X = 20% |
| Prime | X = 18% |

### Survivor's Edge

![survivors_edge](../../../../../Project/public/assets/incarnon-evolutions/XCritChanceYStatChance(xWhite).png)

**Efectos:**
- Increase Base Critical Chance by **+X%**
- Increase Base Status Chance by **+Y%**

| Variante | Valor |
|----------|-------|
| Boltor | X = 8% / Y = 8% |
| Telos | X = 4% / Y = 4% |
| Prime | X = 10% / Y = 10% |

### Commodore's Fortune

![commodores_fortune](../../../../../Project/public/assets/incarnon-evolutions/HighCrit(xWhite).png)

**Efectos:**
- Increase Base Critical Chance by **+X%**

| Variante | Valor |
|----------|-------|
| Boltor | X = 12% |
| Telos | X = 6% |
| Prime | X = 14% |
