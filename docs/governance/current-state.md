...
| Enemy / target model (C2) | 🟡 **Eje arrancado (2026-07-06) — contraste #0 (escalado) COMPLETO** — `EnemyRepository.scale()` = curva-S real por facción (health validado exacto contra el calculador del wiki) + floor/cap de armor + `damageReductionFromArmor` (`√3a/100`, **provisional** `OQ-ENGINE-15` — conflicto de 3 vías en la wiki, se resuelve con #1). Fixture Arid Butcher (`fixtures/enemies.ts`), `enemy-scaling.test.ts` valida health/armor/DR/EHP @215. El juego NO muestra HP numérico → el oráculo es el calculador (gadget) hasta #1. **Diferido:** el **consumo** en el pipeline de daño (facción × DR × capa) = contraste **#1** (popup real); shields-scaling, Eximus, Steel Path, Empowered, overguard, affinity (fórmulas capturadas en `enemy-level-scaling.md`, no en el engine); `processDots` (N-timers, ver `damage-status-model.md`). Data: Arid Butcher es fixture a mano — `Enemy.json` sin cablear al pipeline aún |
