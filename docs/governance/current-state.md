...
| `core/engine/formulas/` | **SSoT matemático pretendido — parcialmente integrado** | **NO está vacío:** 19 archivos con matemática real — `common/{crit-base,scaling-base,status-base}`, `status/{stack-debuff,proc-selection,dot-tick,dot-timeline,proc-population,dot-population}` (2026-07-10/11, Familia A + selección de proc + valor de tick DoT/Familia C + superposición de pulsos declarados + eje Población/RNG, `arch-decisions §14` + `damage-status-model §Modelo de timeline`/`§Población/RNG`), `weapon/{weapon-crit,weapon-status,weapon-multishot,weapon-condition-overload,melee-combo,sniper-combo}`, `enemy/{enemy-scaling,armor-mitigation}`, `ability/{ability-crit,ability-status}`. El principio (arch `simulation-architecture §C1`): `formulas/` = matemática pura que `resolution/`+`combat/` deben *orquestar*. **Deuda conocida:** durante v2 el motor reimplementó matemática inline en vez de consumir `formulas/` → dos sistemas paralelos. **Integrado:** `crit-base` ← `AtomicSimulator`, `scaling-base` + `weapon-condition-overload` (`coBonusPct`) + `weapon/{melee-combo,sniper-combo}` ← `SimulationEngine`, `enemy/{enemy-scaling,armor-mitigation}` ← `EnemyRepository.scale` (orquestador; **P1 2026-07-09: matemática movida de `EnemyRepository`**), **`status/stack-debuff` (Familia A) ← `EnemyState` orquestador (2026-07-10: LEY extraída de `getDamageMultiplier`/`getEffectiveArmor`)**, **`status/{dot-tick,dot-timeline,proc-population,dot-population,proc-selection}` ← `EnemyState`/`TimelineSimulator` (2026-07-12, Familia C — Toxin+Slash: `dot_pools` retirado para esos 2 tipos en favor de `active_pulses`, pulsos declarados resueltos vía `CombatSimulator.resolveDamageInstance`; Slash con `bypassArmorAndMatrix` — True bypasea matriz③+DR, no Viral; ver `design/formulas-integration.md §9`)**. **Sin consumir (código muerto a reconciliar):** `weapon/{weapon-crit,weapon-status,weapon-multishot}`, `status-base` (huérfano salvo vía `proc-selection`) — overlap sin resolver entre `weapon-status` y `status/proc-population` (2 implementaciones paralelas de `chance × peso`, la 3ra —`CombatCalculator.project` inline— sigue siendo la única en producción salvo para Toxin/Slash; ver `design/formulas-integration.md §2`), `status/{dot-tick,dot-timeline,proc-population,dot-population}` **para Heat** (frontera 1, pool consolidado genuino — NO el mismo patrón) — **Electricity/Gas fuera a propósito** (frontera 3, cadena/nube), `ability/*`. Plan y auditoría vivos: [`design/formulas-integration.md`](../domains/engine/design/formulas-integration.md). (`arcane/arcane-core.ts` purgado 2026-06-11, shape stale.) |
...
| Enemy / target model (C2) | 🟡 **Eje arrancado (2026-07-06) — contraste #0 (escalado) COMPLETO** — `EnemyRepository.scale()` **orquesta** `formulas/enemy/{enemy-scaling,armor-mitigation}` (P1 2026-07-09): curva-S real por facción (health validado exacto contra el calculador del wiki) + floor/cap de armor + `damageReductionFromArmor` (`√3a/100`, **provisional** `OQ-ENGINE-15` — conflicto de 3 vías en la wiki, se resuelve con #1). Arid Butcher (cargado desde `enemies.json`, no fixture), `enemy-scaling.test.ts` valida health/armor/DR/EHP @215. El juego NO muestra HP numérico → el oráculo es el calculador (gadget) hasta #1. **Diferido:** el **consumo** en el pipeline de daño (facción × DR × capa) = contraste **#1** (popup real); shields-scaling, Eximus, Steel Path, Empowered, overguard, affinity (fórmulas capturadas en `enemy-level-scaling.md`, no en el engine); `processDots` para Heat (N-timers, ver `damage-status-model.md`) — **Toxin+Slash reconciliados (2026-07-12):** `active_pulses` reemplaza `dot_pools` para esos 2 tipos, tick resuelto vía `CombatSimulator.resolveDamageInstance` (Slash bypasea matriz③+DR, True). Electricity/Gas fuera a propósito (frontera 3). **LEY de status (2026-07-10, `arch-decisions §14`):** Familia A (Infection/Corrosion/Disruption stacks) extraída inline→`formulas/status/stack-debuff.ts`; `EnemyState` = orquestador; **ESTADO keyeado por EFECTO** (`EnemyStatusState`: corrosion/infection/ignite/disruption, ya no por tipo de daño). Marco de flujo del daño en `design/damage-flow-model.md`; lo gated (DamageInstance/Arista 2/Familia C/O4-Magnetic) en `decision-frontier.md §4`. **Rediseño de proc (2026-07-13):** los 3 contenedores (`stacks`/`dot_pools`/`active_pulses`) + `StatusEngine` + `dot_key`/`DotType` se reemplazan por un contenedor único + `EffectBehavior` por efecto — arquitectura RESUELTA en `design/damage-status-model.md §Modelo unificado de proc`; código aún pre-rediseño (drift docs>código deliberado). **Data (2026-07-08): `Enemy.json` cableado al pipeline** por el puerto "0" (`generate-enemies.mjs` → `enemies.json` 638 → `EnemyRepository.load`, `base_level=override??1`; `*_type` per-clase dropeados = deprecados). **`FACTION_BONUS`** (matriz U36 en `contracts/damage-multipliers.ts`) = modelo vigente de daño-vs-facción, reemplaza el per-clase `DAMAGE_EFFICIENCY` (deprecado); **consumida desde `8b014f6` (2026-07-09)** — `resolveHit` usa `targetFactionMult` (matriz③) + `damageReductionFromArmor` (DR), commiteado (checkpoints 1-2 de la reconciliación, `damage-status-model.md §Reconciliación de resolveHit`). `DAMAGE_EFFICIENCY` queda candidato a sunset (cero consumidores reales) — RED, no ejecutado. **Ojo:** esto es acoplamiento *dentro* de C2; C2 (`combat/`) sigue **fuera** del pipeline de producción (`design/formulas-integration.md §1` — el camino vivo es solo C1) |
