...
| `core/engine/formulas/` | **SSoT matemático pretendido — parcialmente integrado** | **NO está vacío:** 9 archivos con matemática real — `common/{crit-base,scaling-base,status-base}`, `weapon/{weapon-crit,weapon-status,weapon-multishot,weapon-condition-overload}`, `ability/{ability-crit,ability-status}`. El principio (arch `simulation-architecture §C1`): `formulas/` = matemática pura que `resolution/`+`combat/` deben *orquestar*. **Deuda conocida:** durante v2 el motor reimplementó matemática inline en vez de consumir `formulas/` → dos sistemas paralelos. **Integrado (Fase 2):** `crit-base` ← `AtomicSimulator`, `scaling-base` ← `SimulationEngine`. **Sin consumir (código muerto a reconciliar):** `weapon/*` (incl. `weapon-condition-overload`), `status-base`, `ability/*`. Plan y auditoría vivos: [`design/formulas-integration.md`](../domains/engine/design/formulas-integration.md). (`arcane/arcane-core.ts` purgado 2026-06-11, shape stale.) |
...
| Condition Overload / GunCO (daño per status type) | 🟡 **Modo estático resuelto (2026-07-04, shotgun)** — `co_behavior` (topología por ataque, resuelto al perfil en `SimulationEntity`) + operation `CONDITION_OVERLOAD` con `co_factors` nombrados; cálculo vía `coBonusPct` (formulas/weapon, no duplicado). Contratos: `MutatedDNA.co_behavior` + `SimulationEntity.co_behavior` + `Modifier.co_factors` + `CoBehavior` SSoT en `@shared/types/modifier`. Los 3 buckets (adding/multiplying/none) ejercitados end-to-end sobre Cedo. Diferido: modo dinámico (N/stacks reales vía `EnemyState`), y resto de la familia CO (rifle/secondary/melee/incarnon — solo verificar datos). Ver `engine/design/arch-decisions.md §9` + `data/decisions.md` D-17 |
