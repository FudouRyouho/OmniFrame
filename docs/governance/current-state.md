...
| `core/engine/hooks/` | **PURGADO (2026-06-16)** | El cluster `useSimulation`/`useSimulationMetrics`/`useTimeline` (D reactiva parcial co-ubicada en `@core`) era código muerto — purgado completo (Fase 0 saneamiento). D se cablea vía `useViewModel` (`@providers`), ver §Shared / `DC-OQ-ENGINE-9`. |
