...
## OQ-DATA-10 — Borde de salida: capa de proyección como SSoT display (espejo de "0") — **ABIERTO — DIFERIDO (2026-06-12); faceta SSoT-vocabulario CERRADA por D-7 Fase 4 (2026-06-14)**
...
- **Proyector engine→display:** ¿`attribute-registry` (keyed por id de engine, con `unit`) es la semilla del proyector que `UpgradeView` debería consumir? ~~Desenchufar el leak `StaticHydrator → lib/presentation`~~ **✅ leak desenchufado (D-7 Fase 4):** `attribute-registry` es ahora `Partial<Record<Upgrade, PresentationMeta>>` keyed por token, consumido por `project()` en el borde — no por el engine. Pendiente: que `UpgradeView` deje su proyección inline y consuma esto vía el contrato D.
...
- **SSoT de label+unit por stat:** ¿cuál de los 3 vocabularios es la fuente y los demás derivan? ¿`stat-labels` (catálogo) y `attribute-registry` (engine) convergen o cubren dos espacios de id distintos? **✅ Ejecutado (D-7 Fase 4, 2026-06-14):** la fuente del lado **engine** es el vocabulario canónico `Upgrade`. El dict se cuelga de él, pero como `Partial<Record<Upgrade, PresentationMeta>>` keyed por **node-id** (NO el `Record` exhaustivo del plan: los FLAT/BASE/sub-familia convergen a su nodo ADD y nunca renderizan → forzarles meta era puerta falsa de vocabulario). Y la **label se separó del dict** (`PresentationMeta = { category, unit }`) — la label es i18n (`stat-labels`), se compone en el borde. **Sigue abierto:** la convergencia/separación entre la ruta **catálogo** (`stat-labels`, espacio de id de `@wfcd/items`) y la ruta **engine** (token D-6) — son dos espacios de id; unificarlos (o decidir que cubren bordes distintos) es trabajo de la suite de presentación, diferido.
