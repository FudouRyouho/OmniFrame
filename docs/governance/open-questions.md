...
- **Composición de `condition` (OR/AND):** `condition: string | null` no expresa `A AND B` ni `A OR B`. Evidencia revisada (2026-06-05): **~9 composiciones reales** hoy colapsadas en token-paraguas `string` — 5 OR (`while_sliding_or_aim_gliding`, `on_parkour_maneuver`, `on_shield_or_overguard_break`, `on_bullet_jump_or_double_jump`, `while_aim_gliding_or_sliding`) + 2 AND evento∧estado (`on_hit_incarnon_form`, `on_hit_while_target_affected_by_electricity`) + Melee Afflictions (anidado). Supera D-20 en masa para las formas planas; sigue gateado por D-16 (cobertura) y por el gate de consumidor.
