...
- **Contenedor de ESTADO entidad-neutral** (jugador self-status, companion) — **CAÍDO POR CONSECUENCIA, no por refactor.** El estado nace del escenario consolidado (`simulation-architecture.md` §*El escenario consolidado*): `EnemyState` recibe la **entidad resuelta de C1** y lee de ella sus tres vitales + su facción, así que cualquier participante con esos nodos puede portar estado. Lo único que queda de "enemigo" en la clase es **el nombre**, y el rename espera un segundo portador real (`OQ-ENGINE-8`) — hoy sería vocabulario sin caso. El gate que lo motivaba —primera entidad no-enemigo que porte status— ya no es el camino. **Lo que sigue abierto es el otro lado:** el `neutral-state` como objeto derivado y el `source-state` vivo (arriba).
