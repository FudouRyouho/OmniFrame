...
- **Contenedor de ESTADO entidad-neutral** (jugador self-status, companion) → **subsumido en el `neutral-state` derivado de arriba** (otra derivación del mismo objeto base). El gate que lo motivaba —primera entidad no-enemigo que porte status— **sigue cerrado y ya no es el camino**: la neutralidad cae **por consecuencia** de que el estado nazca del escenario consolidado (`simulation-architecture.md` §*El escenario consolidado*), porque un participante resuelto ya es neutral. Hoy no nace de ahí: `EnemyState` se construye desde `ScaledEnemy`, un objeto que C1 nunca vio — deuda marcada en `EnemyState` y medible (un debuff `ENEMY_*` compuesto en C1 no llega al daño).
