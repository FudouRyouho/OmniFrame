...
| **Overguard** | **sólo Cold** | — | *"Cold — máximo 4 procs"* | ✅ `RECEIVER_MAX_STACKS_BY_LAYER.overguard` — **no es clase sino capa en `t`** (#11), y desde #38 la capa tiene origen: el Eximus que el escenario declara |
...
### Cold — Freeze
Entra: bonus de crit damage recibido, +0.1× (1er stack) +0.05×/stack, techo +0.5× en el 9º — el último stack de la fórmula **continua**. Behavior `freeze`, mismo canal `critModifier` que Puncture (`DC-OQ-ENGINE-12`).

**El 10º stack no extiende la fórmula: la reemplaza** (#12). Al alcanzarlo se dispara la congelación sólida (3 s), el crit recibido pasa al **fijo +1.0×**, el contador deja de aceptar procs nuevos, y al expirar colapsa a **3 residuales** — no a 0. Por eso `COLD_CRIT_LAW.cap = 0.5 = f(9)` nunca se evalúa en `n=10`: el umbral intercepta antes. El cap de 4 en Overguard (#11) queda por debajo del umbral, así que un target con la capa activa jamás llega a congelarse.

**Ausencias vivas:** el cap de 4 en **bosses** sigue sin portador (`Boss` vetado por §22, ver #39). Diferido: slow (survivability). Fuera del dominio del motor: *"sin acciones"* (control de input) y *"niega recarga de shields"* (no hay sistema de regen de shields, con o sin el proc).
...
- Un canal de **emisión** + un **modificador de resolución** genérico (armor + layer). Armor NO es canal
  ...
  (behaviors `weakened`/`freeze`, `DC-OQ-ENGINE-12`). Fidelidad de suelo; las dos ausencias que se
  destilaron a Issues ya cerraron —cap-Overguard (#11) y 10º stack (#12)—; Puncture/AoE sigue gateado en
  `OQ-ENGINE-12` y el cap de bosses en #39.
...
- **OQ-ENGINE-12** — el gancho de crit (Puncture/Cold) se **construyó** (`DC-OQ-ENGINE-12`, canal `critModifier`); lo único que sigue vivo es Puncture/AoE-habilidades, gateado hasta que exista AoE. Cold cap-4-Overguard y freeze 10º stack se destilaron a Issues y **los dos cerraron** (#11, #12).
