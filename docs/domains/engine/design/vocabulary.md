...
| `EnemyState` | **`EntityState`** — el estado base de una **entidad**: si un portador trae una particularidad, extiende este conjunto en vez de fundar otro | ✅ ejecutado: **76 identificadores** en 28 archivos de `Project/src` + `Project/scripts`, **63 menciones** en 14 docs, y el archivo (`simulate/enemies/EnemyState.ts` → `EntityState.ts`). **NO** `engine-audit.md` (histórico: dice el nombre de su época y es correcto *como registro*). ⛔ **`NeutralState` se descartó y no por gusto**: el corpus ya lo reserva para la base de la que derivarían los estados **por naturaleza del nodo** —source / target / minion / object—, donde esta clase figura como **uno de los derivados** (`decision-frontier §4`, `arch-decisions §15`, sin construir). Usarlo acá colapsaba padre e hijo. ⛔ `BearerState` tampoco: `withBearer`/`bearerIntent` ya usan *portador* para el que lleva un slot. **Y el archivo salió de `simulate/enemies/` a la raíz de `simulate/`**, junto a `advance.ts` (el bucle que lo avanza): esa carpeta particiona por **entidad** y `combat/` por **proceso**, y este archivo no entra en ninguno. Regla que queda: **la raíz de `simulate/` es lo que no es de ninguna entidad**. `EnemyRepository` (su dato es `enemies.json`) y `EnemySnapshot` (hoy lee el target) se quedan. |
...
| ID | Colisión | Estado |
|---|---|---|
| **L-3** | **`pool` sobrecargado**: además del grupo aditivo, significa **contenedor de instancias de proc** (`HeatState.pool` con decay en `behaviors.ts`, dentro del estado por-efecto de `EntityState`). | abierta |
...
| **L-13** | **`WEAPON_ADD_<TIPO>_DAMAGE` nombraba dos cosas**: (a) el **bucket de upgrade** —*dónde suma Hellfire*, que **es del arma** y ahí el prefijo es fiel— y (b) el **tipo de la instancia** —*estos 400 puntos son Heat*, que no es de nadie. Las tres leyes de resolución usaban (a) para responder (b), así que un emisor no-arma las perdía **sin ruido**: matriz de facción `1500 → 1000`, Toxin cayendo en `shield` en vez de `health`, True `1000 → 99.99`. | **cerrada** — (b) pasó a `DamageType` (`damage-logic.ts` §los dos trabajos, `arch-decisions §18` cuarta aparición); (a) se quedó con el token. **No se acuñó vocabulario nuevo**: `AVATAR_ADD_<TIPO>_DAMAGE` no existe en el dato de DE (0/116) y no debe inventarse. Lo que sí ganó nombre es la **clase del emisor** (`Weapon Damage Ability`, `contracts/emitter-class.ts`) — otra pregunta, y la trae la fuente. |
