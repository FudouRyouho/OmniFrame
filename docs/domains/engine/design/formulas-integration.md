...
| `enemy/effective-health.ts` | `effectiveHealthVsEnemy` — `Σ_capa cantidad×factor` sobre `LAYER_STACK`, con `factor(health)=1/(1−DR)`, componiendo la DR adoptada | **enemigo** | ✅ consumido por la lente `enemy` del oráculo. Ya **no enumera** capas: suma las de `contracts/layers.ts` (#55 — faltaba `overguard` y sumó cero mientras la capa valía cero). `armor` entra aparte porque no es capa; `health` obligatorio, el resto opcional en 0 |
...
| `ability/ability-crit.ts` | `calculateGyreCrit`, `hasAbilityCritException` | agnóstico | ✅ **wired** ← `AbilityRepository.getEmissions` (#9). No es fórmula genérica: es la excepción de **una** warframe (Gyre), aproximación v1 (+10% flat) — el mecanismo real es condition-scaled, sin verificar (#33) |
