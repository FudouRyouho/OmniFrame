---
Estado: "activo"
Rol: "Auditoría de alineación entre docs/design/sim-v2/ y la implementación real del motor"
Version: "v0.1.0"
Impacto_ID: "E-01"
Fidelidad_Fisica: "Project/src/core/engine/sim-v2/"
...
**Alcance**: Lectura completa de `Project/src/core/engine/sim-v2/` comparada contra los cuatro documentos de diseño.
...
| MutatorBridge — doble ruta | `simulate(LoadoutState)` + `simulateFromIntention(EnsembleIntention)` | Ambas rutas operativas vía `EnsembleAdapter`. |
...
En ambas rutas de `EnsembleAdapter`:
```typescript
...
| :--- | :--- | :--- |
| DNA Mutation Step (Archon Shards, Helminth) | `simulation-architecture.md §Capa B` | `shards: []` y `helminth: undefined` hardcodeados en `EnsembleAdapter`. La mutación de ADN nunca ocurre. |
...
| Companion channel | `simulation-contracts.md §Capa A` | No incluido en `EnsembleAdapter.toEnsemble()` ni `fromIntention()`. |
...
Piezas que existen en el código pero que no tienen contraparte en `docs/design/sim-v2/`.
