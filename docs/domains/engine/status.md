...
| `layers.ts` | **Activo** — la pila defensiva (`overguard → overshield → shield → health`) y **la tabla de quién la atraviesa, que es de la capa y no del daño**. Declara la LEY; de las cuatro, la única sin ningún origen construido es `overshield` (el Overguard ganó el suyo con #38, camino Eximus — los otros dos que la fuente nombra siguen sin construir). ⚠️ Su tabla tiene **un eje donde la realidad tiene una pregunta**: `Toxin vs shields` no discrimina por clase pero `Overguard vs status` sí (jugador los niega, enemigo los recibe) — ver `time-model.md §8`. |
...
| `ability/` | `ability-crit` (**wired** → `AbilityRepository.getEmissions`, aproximación v1 — sólo Gyre), `ability-status` |
...
`|| 1` le daría una cadencia inventada de 1/s en vez de sonar, que es la **tercera** vez que este mismo
`|| 1` aparece en la lista de arriba. Refuerza la cura de fondo: quien emite declara **cuándo**.
