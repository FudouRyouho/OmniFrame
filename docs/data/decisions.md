...
| `WEAPON_DAMAGE_IF_VICTIM_PROC_ACTIVE` | ✅ **RESUELTO 2026-07-04 (shotgun).** Familia Condition Overload / GunCO. Token `WEAPON_ADD_DAMAGE_PER_STATUS_TYPE` con `op: CONDITION_OVERLOAD` + `co_factors: {stacks_var, status_count_var}` (dos dimensiones nombradas, asumidas en estático / emergentes en dinámico). El valor lo calcula `coBonusPct` (formulas/weapon); el bucket lo decide el `co_behavior` del ataque (adding/multiplying/none), NO la operación. Re-map: Galvanized Savvy (shotgun) + Aptitude (rifle) [drift: rank-10 80 = clon del Status Chance] + Galvanized Shot (secondary) [drift: rank-10 120 = coefBase×maxStacks(40×3) bakeado]. Los tres → coefBase **40** contra DE; `maxStacks` difiere (Savvy/Aptitude=2, Shot=3) pero el motor usa el `active_stacks` declarado, no lo hornea. **Perks incarnon (8: Angstrum, Burston, Despair, Lato, Soma, Zylok, Dual Toxocyst=33, Latron=30+sprint) resueltos 2026-07-04**: se les añadió `upgrade_type` (antes AUSENTE) + IncarnonRepository propaga `co_factors`; son **1x (sin stacks)** — coefBase directo, sin bakeo — y `active_stacks` no se declara → default 1. **Condition Overload melee resuelto 2026-07-04** (tras el hit-base melee, OQ-ENGINE-14): coefBase **80**, 1x; `co_behavior=adding` vía `kind=melee` (Normal Attack shot_type=None → default melee); condition `per_status_type_on_target` removida (era la escala N disfrazada). **✅ FAMILIA COMPLETA** — el token legacy ya no aparece en ningún `upgrade_type` de override (solo en `mods.json` crudo upstream y notas históricas). Ver `arch-decisions.md §9`. | — |
...
- `WEAPON_DAMAGE_IF_VICTIM_PROC_ACTIVE` → ✅ `WEAPON_ADD_DAMAGE_PER_STATUS_TYPE` en `UPGRADES[]` (2026-07-03, shotgun). Ya no se dropea para Galvanized Savvy; motor lo evalúa (modo estático/techo). Resto de la familia CO sigue con el token legacy (dropeado) hasta verificar sus datos.
