...
- `pipeline:debt` **Campos disponibles en upstream que `generate-data.ts` descarta** (censo sobre `All.json`/`Node.json`): `baseDrain`/`fusionLimit` (1.784 mods) · `excludeFromCodex` (2.890 ítems — mejor filtro de entidades que el `ExcludedFromSimulacrum` del wiki) · `minEnemyLevel`/`maxEnemyLevel`/`factionIndex` (los 269 nodos del Star Chart, único dato de enemigos fresco que sobrevive al fósil de 2019 — permitiría anclar el nivel de simulación a misiones reales) · `probability`/`tier` de sorties (17) · extractores (6). Ninguno llega a `public/data`. Detalle y censo en `../domains/source/gaps.md` §G-3 `[empirical]`
