...
| Tipo OmniFrame D-6 | Evidencia | Modelo | Ejemplo de mod |
| :--- | :--- | :--- | :--- |
| `WEAPON_ADD_ACCURACY` | `[ref: accuracy.md]` | `—` | Pax Soar (arcane); exilus On-Equip. `WEAPON_SPREAD` (DE legacy) confirmado = **mismo stat** (spread = inverso de accuracy; Narrow Barrel / Tainted Shell llevan token spread con label "+% Accuracy") → unificar bajo este token. Sim asume aim perfecto — stat informativo. |
...
| **95 tokens / 239 usos** fuera de `UPGRADES` | Stats reales del juego que el proyecto no modela (`WEAPON_SYNDICATE_POWER` 24, `WEAPON_SPREAD` 17, `AVATAR_DAMAGE_TAKEN` 13, `AVATAR_PARKOUR_*`, …). Cola larguísima: ~70 con **1 uso**. Hoy son **invisibles** — ése es el defecto que este registro corrige. | Caso por caso, con el criterio de D-20. |
