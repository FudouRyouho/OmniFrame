...
| **7 tokens `AVATAR_*` que no son tokens** — los 5 `AVATAR_SENTINEL_{ARMOUR,CRIT,HEALTH,SHIELD,STATUS}_LINK` (Link Vitality, Link Fiber, **Link Redirection**, Hunter Synergy, Mecha Overdrive) + `AVATAR_NPC_HIT_CHANCE` (Emp Aura) y `AVATAR_MARKED_DAMAGE_AMOUNT` (Mecha Empowered), 1 uso cada uno | Fallan el **test del eje nuevo** de arriba, por dos motivos distintos. Los `*_LINK` meten el eje **leer de otra entidad** —[`../data/rules/overrides.md`](../data/rules/overrides.md) lo clasifica como **relacional → fórmula dedicada**—: no es un valor nuevo del eje, es un eje que el token no puede portar. Los otros dos ponen un efecto **contra el enemigo** bajo prefijo `AVATAR_`: el `{dónde}` miente sobre el sujeto. | **No es migración** —ya están fuera de `UPGRADES` y gritan en hidratación—: es **darles nombre**. Fórmula dedicada para los `*_LINK`; para los dos hostiles, decidir su sujeto real antes de acuñar nada. El mecanismo para eso ya existe (`source_entity` + `source_attribute`, el de Roar) y **alcanza**: el vínculo es de máximo a máximo, `Warframe.max → Companion.max`. Lo que parece un vínculo de **estado** —[`link-redirection.wikitext`](../../references/wiki/mods/link-redirection.wikitext) §Notes dice que el mod *"allow[s] most direct forms of shield recovery to apply to the pet"* (Condemn, Pillage, Capacitance, Grenade Fan)— **no lo es**: [`link-vitality.wikitext`](../../references/wiki/mods/link-vitality.wikitext) §Patch History `ver|34` declara removida la propagación de curación *"never explained what forms of healing worked or why"* y, en la misma nota, levanta las restricciones de 14 habilidades *"as they apply to granting Health, **Shields**, or Damage Resistance to Companions"*. Post-`ver|34` esas restauraciones llegan al compañero **porque es un aliado**, no por el mod; lo que el mod aporta es el **contenedor** (un pool de máximo grande donde el efecto general puede entrar), y por eso la misma página aclara que no crea overshields ni comparte regen pasiva. **Dos mecanismos, no uno**. |
