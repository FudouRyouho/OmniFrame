...
- **Entrada**: Archivos de captura en `docs-references/semantic-ui-rips/*.md`.
- **Procesamiento**:
  - Parsea la jerarquía de `##` (Habilidad), `###` (Subgrupo/Modo) y `####` (Augment).
  - Extrae valores numéricos básicos y tags semánticos (`<DT_SLASH>`, etc.).
- **Salida**: Genera `docs-references/semantic-ui-rips/parsed-output.json`.
...
### Mantenimiento
Si la gramática del Markdown en `docs-references/` cambia, el script en `utilities/` debe ser actualizado para evitar la pérdida de fidelidad en la captura.
