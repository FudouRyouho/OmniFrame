 ...
 * numérica sobre 628 armas × 1485 ataques + 115 warframes.
 *
 ...
 *   - Helminth: todos los stats = 0 (NPC no equipable en el dataset)
...
describe("Engine smoke — warframes (115)", () => {
	...
	it("calcula todos los warframes sin NaN ni Infinite", () => {
		...
		for (const raw of warframes) {
			if (raw.name === "Helminth") continue; // NPC — todos los stats = 0, skip
	...
	it("Helminth tiene todos los stats base en 0", () => {
		const helminth = warframes.find(w => w.name === "Helminth");
		expect(helminth).toBeDefined();
		const layout: ResolvedLayout = {
			warframe: { base: toWarframeBase(helminth!), stats: [] },
		};
		const out = calculate(layout, CTX);
		expect(out.warframe?.health).toBe(0);
		expect(out.warframe?.shield).toBe(0);
		expect(out.warframe?.armor).toBe(0);
	});
	...
	it("cobertura — 115 warframes procesados", () => {
		expect(warframes.length).toBe(115);
	});
