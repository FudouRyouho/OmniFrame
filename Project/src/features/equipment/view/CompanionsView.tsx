...
const CompanionsView = () => {
    ...
    return (<div className="h-full overflow-hidden">
        <ItemsGrid
            items={data}
            isLoading={isLoading}
            onSelect={(item) => navigate(`/equipment/companions/${toRouteSlug(item.name)}`, { state: { uniqueName: item.uniqueName } })}
            renderItem={(item) => (
                <CompanionCard 
                    key={item.id} 
                    item={item as Companion}
                    onSelect={(c) => navigate(`/equipment/companions/${toRouteSlug(c.name)}`, { state: { uniqueName: c.uniqueName } })}
                />
            )}
        />
        </div>
