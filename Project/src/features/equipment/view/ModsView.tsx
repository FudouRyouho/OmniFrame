...
const ModsView = () => {
    ...
    const renderModCard = (item: Mod) => (
        <ModCard
            key={item.id}
            item={item}
            onSelect={(mod) => navigate(`/equipment/mods/${toRouteSlug(mod.name)}`, { state: { uniqueName: mod.uniqueName } })}
        />
    );
    ...
    return (<div className="h-full overflow-hidden">
        {data.length > MODS_VIRTUALIZATION_THRESHOLD ? (
            ...
            <ItemsGrid
                items={data}
                isLoading={isLoading}
                onSelect={(item) => navigate(`/equipment/mods/${toRouteSlug(item.name)}`, { state: { uniqueName: item.uniqueName } })}
                layout={{
                    minColumnWidth: MODS_CARD_MIN_WIDTH,
                    gap: MODS_GRID_GAP,
                }}
