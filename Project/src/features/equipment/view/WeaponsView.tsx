...
const WeaponsView = () => {
    ...
    const renderWeaponCard = (item: Weapon) => (
        <WeaponCard
            key={item.id}
            item={item}
            onSelect={(weapon) => navigate(`/equipment/weapons/${toRouteSlug(weapon.name)}`, { state: { uniqueName: weapon.uniqueName } })}
        />
    );
    ...
    return (<div className="h-full overflow-hidden">
        {data.length > WEAPONS_VIRTUALIZATION_THRESHOLD ? (
            ...
            <ItemsGrid
                items={data}
                isLoading={isLoading}
                onSelect={(item) => navigate(`/equipment/weapons/${toRouteSlug(item.name)}`, { state: { uniqueName: item.uniqueName } })}
                layout={{
                    minColumnWidth: WEAPONS_CARD_MIN_WIDTH,
                    gap: WEAPONS_GRID_GAP,
                }}
