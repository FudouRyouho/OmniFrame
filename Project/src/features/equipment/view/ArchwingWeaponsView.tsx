...
const ArchwingWeaponsView = () => {
    ...
    return (
        ...
        <ItemsGrid
            items={data}
            isLoading={isLoading}
            onSelect={(item) => navigate(`/equipment/archwing-weapons/${toRouteSlug(item.name)}`, { state: { uniqueName: item.uniqueName } })}
            renderItem={(item) => (
                <ArchwingWeaponCard 
                    key={item.id} 
                    item={item as ArchwingWeapon}
                    onSelect={(aw) => navigate(`/equipment/archwing-weapons/${toRouteSlug(aw.name)}`, { state: { uniqueName: aw.uniqueName } })}
                />
            )}
        />
        </div>
