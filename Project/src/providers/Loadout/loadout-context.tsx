...
import {
	...
	type LoadoutState,
	type ModSlot,
} from "@features/arsenal/engine/loadout";
import { loadResolverDependencies } from "@features/arsenal/engine/runtime-deps";
import {
	resolve,
	type LoadoutInput,
} from "@features/arsenal/engine/resolver";
...
export type LoadoutAction =
	...
	| { type: "set-mod"; channel: LoadoutChannel; slotIndex: number; mod: ModSlot | null }
...
export type LoadoutContextValue = {
	...
	setMod: (channel: LoadoutChannel, slotIndex: number, mod: ModSlot | null) => void;
	replaceLoadout: (nextLoadout: LoadoutState) => void;
	clearLoadout: () => void;
};

const CHANNELS = ["warframe", "primaryWeapon", "secondaryWeapon", "meleeWeapon"] as const;
...
export function LoadoutProvider({ children }: { children: ReactNode }) {
	...
	const [resolverDepsLoaded, setResolverDepsLoaded] = useState<Awaited<ReturnType<typeof loadResolverDependencies>> | null>(null);
	...
	useEffect(() => {
		...
		void loadResolverDependencies()
			...
			.catch((reason: unknown) => {
				...
				setError(reason instanceof Error ? reason.message : "No se pudieron cargar los datasets del builder");
			});
	...
	}), [activeChannelCount, engineOutput, error, isLoading, isReady, loadout, resolvedLayout, resolverInput]);
