...
const arid = () => {
  const dna = EnemyRepository.find('Arid Butcher');
  if (!dna) throw new Error('fixture: Arid Butcher no encontrado en el registro de enemigos');
  return dna;
};
...
describe('CombatMetrics — contrato de salida de C (DC-OQ-ENGINE-8)', () => {
  it('estructura: el conjunto se parte en target_agnostic + vs_target', () => {
    const m = computeCombatMetrics(weapon(), EnemyRepository.scale(arid(), 1), ctx, 6);
  ...
  it('fork (b) — no mata: ttk=null y effective_dps = total_damage / dur (sostenido)', () => {
    ...
    const dur = 3;
    const m = computeCombatMetrics(weapon(), EnemyRepository.scale(arid(), 9999), ctx, dur);
