...
describe('Boltor — Normal Attack base (estabilidad: .final)', () => {
  ...
  it('WEAPON_DAMAGE = 46; CC 12, CM 2, SC 34', () => {
    const w = probe();
    expect(w.node('WEAPON_DAMAGE').final).toBeCloseTo(46, 0);
...
// ─── Lógica: linaje de WEAPON_DAMAGE por bucket ──────────────────────────────────

describe('Boltor — WEAPON_DAMAGE por buckets (lógica: dónde aterriza)', () => {
  it('peldaño 0 — base limpia: todo en base, sin aportes', () => {
    const d = probe().node('WEAPON_DAMAGE');
  ...
  it('peldaño 1 — +Hunter\'s Mantra: el perk en base_flat (NO en mods)', () => {
    const d = probe({ perks: { 2: 'hunters_mantra' } }).node('WEAPON_DAMAGE');
  ...
  it('peldaño 2 — +Serration: el mod en mods_add_pct, amplifica (base+base_flat)', () => {
    const d = probe({ perks: { 2: 'hunters_mantra' }, mods: { 0: SERRATION } }).node('WEAPON_DAMAGE');
...
describe('Boltor — EVO II + EVO IV combinados (perks independientes)', () => {
  it('WEAPON_DAMAGE = 50, CC = 26', () => {
    const w = probe({ perks: { 2: 'hunters_mantra', 4: 'commodores_fortune' } });
    expect(w.node('WEAPON_DAMAGE').final).toBeCloseTo(50, 0);
...
describe('Boltor — genesis activo: base vs incarnon_form (misma build)', () => {
  ...
  it('WEAPON_DAMAGE: perk +4 en base_flat en AMBOS perfiles (final 50 vs 28)', () => {
    const base = probe({ perks: PERKS, profile: 'base' }).node('WEAPON_DAMAGE');
    const inc  = probe({ perks: PERKS, profile: 'incarnon_form' }).node('WEAPON_DAMAGE');
...
describe('Boltor — Incarnon Form base (estabilidad: .final)', () => {
  it('stats del perfil incarnon_form', () => {
    ...
    expect(w.node('WEAPON_DAMAGE').final).toBeCloseTo(24, 0);
...
describe('Boltor — audit trace de WEAPON_DAMAGE (debug)', () => {
  it('reporta procedencia real, perks incluidos', () => {
    const trace = probe({ perks: { 2: 'hunters_mantra' }, mods: { 0: SERRATION } }).audit('WEAPON_DAMAGE');
    console.log('\n[debug] WEAPON_DAMAGE audit trace:');
