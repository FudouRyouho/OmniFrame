 ...
 * sus 5 buckets + el trace de procedencia, opt-in — ver `SimulateOptions`), este clic
 ...
 *
 * (i)  `node()`  → el AttributeNode completo: base + 5 buckets + final. Superficie de aserción limpia.
...
export interface Consumption {
  ...
  /**
   * Volcado crudo: todas las entidades resueltas con sus `AttributeNode` completos (finales + 5 buckets).
